...
def get_maximum_henchmens(lamb):
    if lamb == 1:
        return 1
    else:
        payments = [1,1]
    while sum(payments) <= lamb:
        payments.append(payments[-1]+payments[-2])
    return len(payments)-1

def get_minimum_henchmens(lamb):
    payments = [1]
    while sum(payments) <= lamb:
        payments.append(payments[-1]*2)
    return len(payments)-1

def solution(lamb):
    return get_maximum_henchmens(lamb)-get_minimum_henchmens(lamb)
