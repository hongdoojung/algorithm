...
def find_root(sets, number):
    if sets[number] != number:
        sets[number] = find_root(sets, sets[number])
    return sets[number]
...
def main():
    biggest, how_many = (map(int, input().split()))
    sets = [i for i in range(biggest + 1)]
    ...
    for array in arrays:
        ...
        b = array[2]
        if array[0]:
            if find_root(sets, a) == find_root(sets, b):
                print("YES")
            else:
                print("NO")
        else:
